...
void play_mario_landing_sound(struct MarioState *m, u32 soundBits) {
    play_sound_and_spawn_particles(m, soundBits, 1);
}
...
void play_mario_landing_sound_once(struct MarioState *m, u32 soundBits) {
    play_mario_action_sound(m, soundBits, 1);
}
...
s8 sTerrainSounds[7][6] = { 
    ...
    { SOUND_TERRAIN_SNOW,    SOUND_TERRAIN_DEFAULT,SOUND_TERRAIN_SNOW,
      SOUND_TERRAIN_SNOW,    SOUND_TERRAIN_ICE,    SOUND_TERRAIN_ICE }, // TERRAIN_SNOW
...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    if ((m->squishTimer != 0) && (action == ACT_DOUBLE_JUMP || action == ACT_TWIRLING)) {
        action = ACT_JUMP;
    }
