...
void bowser_act_jump_onto_stage(void) {
    ...
    switch (o->oSubAction) {
        ...
        case BOWSER_SUB_ACT_JUMP_ON_STAGE_START:
            cur_obj_init_animation(BOWSER_ANIM_JUMP_START);
            if (cur_obj_check_anim_frame(11)) {
...
s32 bowser_dead_twirl_up(void) {
    ...
    if (o->header.gfx.scale[1] <= 0.0 && bowserTimer > 0) {
        mist = spawn_object(o, MODEL_SMOKE, bhvWhitePuff1);
        obj_translate_xz_random(mist, 64.0f);
        mist->oPosY -= 70.0f;
    }
    if (o->header.gfx.scale[1] <= 0.0 && bowserTimer < 9) {
        bowserTimer++;
    }
    if (o->header.gfx.scale[1] <= 0.0 && bowserTimer == 9) {
        ret = 1;
...
void bowser_thrown_dropped_update(void) {
    ...
    swingSpd = o->oBowserHeldAngleVelYaw / 3000.0 * 60.0f;
